...
import { 
  ...
  Filter, Download, ArrowDownToLine, ArrowUpToLine, Loader2, AlertCircle, X, Check, Clock
} from 'lucide-react'
...
export const SavingsPage: React.FC = () => {
  ...
  // Query Pending Savings Requests (Admin)
  const { data: pendingRequestsData, isLoading: isPendingLoading } = useQuery({
    queryKey: ['adminSavingsRequests', 'pending'],
    queryFn: () => adminService.getSavingsRequests('pending')
  })
  const pendingRequests = pendingRequestsData?.data || []

  // Approve Mutation
  const approveMutation = useMutation({
    mutationFn: (id: number) => adminService.approveSavingsRequest(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminSavingsRequests'] })
      if (selectedMemberId) {
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }: { id: number; reason: string }) =>
      adminService.rejectSavingsRequest(id, { rejection_reason: reason }),
    onSuccess: () => {
    ...
    >
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Review member requests and record savings deposits/withdrawals
        </p>
      </div>

      {/* Pending Savings Requests Section */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Clock className="w-5 h-5 text-amber-500" />
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">
              Pending Member Requests ({pendingRequests.length})
            </h3>
          </div>
        </div>
        ...
        {isPendingLoading ? (
          ...
          <div className="px-6 py-6 text-center text-sm text-slate-500 dark:text-slate-400">
            No pending savings requests to review.
          </div>
        ) : (
                  ...
                  <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-6 py-4 capitalize font-medium">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-semibold ${
                        ...
                        {req.type === 'deposit' ? <ArrowDownToLine className="w-3 h-3" /> : <ArrowUpToLine className="w-3 h-3" />}
                        {req.type}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-xl p-6 shadow-xl border border-slate-200 dark:border-slate-800">
            ...
            <textarea
              ...
              placeholder="e.g. Invalid transaction reference or insufficient documentation"
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              <button
                type="button"
                onClick={() => rejectMutation.mutate({ id: rejectingRequestId, reason: rejectionReason })}
